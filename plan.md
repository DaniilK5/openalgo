...
- `APP_TIMEZONE` is configurable in `.env` and defaults to `Asia/Almaty`. Backend/frontend date displays and Bybit order/trade timestamps use it. New and edited schedules use the application timezone; existing recurring schedules preserve their execution instants, with Historify's migration backfilling legacy rows as `Asia/Kolkata`.
